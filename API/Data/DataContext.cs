...
public class DataContext(DbContextOptions<DataContext> options) : IdentityDbContext<User, IdentityRole<int>, int>(options)
{
 ...
 .HasPrecision(18, 6);
    ...
    // Stock indexes
    modelBuilder.Entity<Stock>()
        .HasIndex(s => s.Symbol);  // Fast lookups by stock symbol
        
    modelBuilder.Entity<Stock>()
        .HasIndex(s => s.isETF);    // Filter by stock type
        
    // Stock price indexes
    modelBuilder.Entity<StockPrice>()
        .HasIndex(sp => new { sp.StockId, sp.Date });  // For historical price queries
        
    // Forum indexes
    modelBuilder.Entity<ForumThread>()
        .HasIndex(ft => ft.CreatedBy);  // Find threads by user
        
    modelBuilder.Entity<ForumThread>()
        .HasIndex(ft => ft.CreatedAt);  // For sorting/filtering by date
        
    modelBuilder.Entity<ForumMessage>()
        .HasIndex(fm => fm.ThreadId);   // Fast message lookup by thread
        
    modelBuilder.Entity<ForumMessage>()
        .HasIndex(fm => fm.UserId);     // Find messages by user
        
    // Alert indexes
    modelBuilder.Entity<Alert>()
        .HasIndex(a => new { a.UserId, a.StockId });  // For user-specific alerts
        
    modelBuilder.Entity<Alert>()
        .HasIndex(a => a.TargetPrice);  // For price comparison queries
    
    // Watchlist indexes
    modelBuilder.Entity<WatchListStock>()
        .HasIndex(w => w.StockId);
   }
